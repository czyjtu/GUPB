...
from gupb.controller.r2d2.r2d2_helpers import walking_distance, get_cut_positions, get_possible_attack_positions
...
class ChaseWeaker(Strategy):
    ...
    def decide(self, knowledge: R2D2Knowledge, state_machine: R2D2StateMachine) -> Action:
        ...
        # # remove enemy cut positions from the list 
        # enemy_cuts = get_cut_positions(self.target_coord, self.target_champion, knowledge)
        # safe_cut_positions = list(filter(lambda x: x not in enemy_cuts, cut_positions))
        # if safe_cut_positions:
        #     closest_position, dist = find_closest_position(my_coords, safe_cut_positions, knowledge)
        # find the closest position where we can attack the target
        # else:
        ...
        cut_coordinates = [coords for coords in cut_coordinates if walking_distance(my_coords, coords, knowledge.world_state.matrix_walkable) == dist]
